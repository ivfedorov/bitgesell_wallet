...
case Constants.Networks.Testnet:
    ...
    break;
case Constants.Networks.Bitgesell:
...
const getFee = () => {
    return axios.get(Constants.Endpoints.BitcoinFees).then((response) => {
        return (response.data.fastestFee * Constants.Transactions.AverageBytes) / Constants.Bitgessel.Satoshis;
    }).catch(() => {
        return 0;
    });
};
...
const getUnspentOutputs = (address) => {
    return c_blockexplorer.getUnspentOutputs(address).then((result) => {
        return {
            ...
            coins: result.unspent_outputs.reduce((a, c) => a + c.value, 0) / Constants.Bitgessel.Satoshis
        };
    });
};
