...
class Wallet extends EventEmitter {
     ...
     */
    get coins() {
        //return this.utxos.reduce((a, c) => a + c.value, 0) / Constants.Bitgessel.Satoshis;
        return this.__utxos[0].amount; //((a, c) => a + c.value, 0) / Constants.Bitgessel.Satoshis;
    }
    ...
    send(btc, address, fee, password) {
        ...
        const satoshis = Math.round(btc * Constants.Bitgessel.Satoshis);
        const satoshis_fee = Math.round(fee * Constants.Bitgessel.Satoshis);
    ...
    update() {
        fetch('http://localhost:5000/get_utxo?address='+this.address)
            .then(response => response.json())
            .then(
                // handle the result
                (result) => {
                    this.utxos = result.result.unspents;
